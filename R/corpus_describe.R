...
collect_corpus_summary <- function(corpus_obj, verbose = FALSE) {
  ...
  list(
    ...
    project = proj,
    generated_by = paste0("reindeer ", as.character(utils::packageVersion("reindeer"))),
    generated_at = format(Sys.time(), "%Y-%m-%dT%H:%M:%S%z"),
    state_hash = tryCatch(.metadata_state_hash(corpus_obj),
...
.emit_readme <- function(summary, path) {
  ...
  if (NROW(summary$participants) > 0) {
    ...
    # List the attribute fields actually recorded, so all captured metadata is
    # visible (not just age/gender).
    attr_fields <- if (is.data.frame(summary$participants)) {
      setdiff(names(summary$participants), c("id", "session", "bundle"))
    } else {
      unique(unlist(lapply(summary$participants,
                           function(p) setdiff(names(p), "id"))))
    }
    if (length(attr_fields) > 0) {
      add("- Attributes recorded: ", paste(sort(attr_fields), collapse = ", "))
    }
    add("")
  }
  ...
  add("---")
  add("")
  stamp <- paste0("_Generated by ", summary$generated_by %||% "reindeer",
                  " on ", summary$generated_at %||% format(Sys.time()), "._")
  add(stamp)
  if (!is.null(summary$state_hash) && !is.na(summary$state_hash)) {
    add("")
    add("_Metadata state: `", substr(summary$state_hash, 1, 12), "`._")
  }
...
.emit_datacite <- function(summary, path) {
  ...
  # Version the artifact by the metadata-state hash so drift is visible in the
  # DataCite record (B5).
  if (!is.null(summary$state_hash) && !is.na(summary$state_hash)) {
    doc$data$attributes$version <- substr(summary$state_hash, 1, 12)
  }
  jsonlite::write_json(
