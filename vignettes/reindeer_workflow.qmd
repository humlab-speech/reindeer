...
The `query()` function (alias: `query()`) implements the EMU Query Language (EQL) with SQL-based optimization:

```{r basic-queries}
# Simple label matching
vowels <- visp_corpus %>% 
  query("Phonetic =~ [aeiou]")

print(vowels)

# Sequence queries
nasal_stop_sequences <- visp_corpus %>% 
  query("[Phoneme =~ [mn] -> Phoneme =~ [ptk]]")

# Dominance queries (hierarchical relationships)
word_initial_stops <- visp_corpus %>% 
  query("Start(Word, Phoneme) =~ [ptk]")
...
vowels <- visp_corpus %>% query("Phonetic =~ [aeiou]")
syllables <- visp_corpus %>% query("Syllable =~ .*")
...
| Query Type | emuR::query | reindeer::query | Speedup |
|------------|-------------|-------------------|---------|
