...
```{r correctness}
# Summary of correctness from test suite
correctness_summary <- data.frame(
...
correctness_summary %>%
  gt() %>%
  tab_header(
    title = "Correctness Verification Summary",
    subtitle = "Test suite results"
  ) %>%
  cols_label(
...
For detailed test results, see `TESTING_RESULTS.md` in the package root.
