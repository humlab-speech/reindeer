...
meta <- get_metadata(ae)
young_speakers <- meta[meta$Age < 30, c("session", "bundle")]

# Query only those bundles
results <- ask_for(ae, "Phonetic == s")
```
...
## Advanced: Metadata Inheritance Inspection

To see the effective metadata for a specific bundle (with inheritance resolved):

```{r inheritance, eval=FALSE}
# Get all metadata with inheritance resolved
all_meta <- get_metadata(ae)

# Filter to specific bundle
effective <- all_meta[all_meta$session == "S1" & all_meta$bundle == "B1", ]
print(effective)

# The returned values reflect the precedence: bundle > session > database
# To inspect raw values at each level, read the .meta_json files directly:
# - Database: <db_emuDB>/METADATA.json
# - Session:  <db_emuDB>/<session>_ses/METADATA.json
# - Bundle:   <db_emuDB>/<session>_ses/<bundle>_bndl/METADATA.json
```
