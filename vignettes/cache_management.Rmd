...
Reindeer manages a single cache directly:

1. **Quantify/Enrich Cache** (`.quantify_cache/`): Stores results from `quantify()` and `enrich()` operations

> **Note:**
> * Draft annotation caches (`.draft_cache/`) are managed by the [protoscribe](https://github.com/humlab-speech/protoscribe) package as of v0.3.0.
> * Simulation result caches (sqlite files produced by `quantify_simulate()` and `enrich_simulate()`) are managed by the [erodex](https://github.com/humlab-speech/erodex) package — see `erodex::list_simulations()` and the `erodex` cache-management utilities.
...
**Warning Thresholds:**
- Quantify/Enrich cache: 500 MB (warning), 2 GB (critical)
...
Simulation caches can grow very large. Manage them with the erodex
package — see `?erodex::list_simulations` and the simulation
cache-management utilities in erodex.
...
Simulation caches (sqlite files) live wherever erodex's `quantify_simulate()` /
`enrich_simulate()` was told to write them via `.simulation_store`, typically
...
- `vignette("simulation-infrastructure", package = "erodex")` - Simulation caching in erodex
